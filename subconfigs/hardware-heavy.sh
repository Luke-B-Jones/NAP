...
# Convert all_RAM from bytes to gigabytes for ease of use
RAM_GB=$(echo "scale=2; $all_RAM / (1024^3)" | bc)

# Adjusting for heavy usage
cores=$(echo "scale=0; $all_cores * 0.95 / 1" | bc)
RAM=$(echo "scale=2; $RAM_GB * 0.95 / 1" | bc)  # Use RAM in GB for calculations
Q_cores=$((cores / 2))
